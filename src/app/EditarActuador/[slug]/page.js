...
const EditarActuador = ({ params }) => {
  ...
  useEffect(() => {
    const infoActuador = async () => {
      try {
        const response = await fetch(`/api/actuadores/${slug}`, {
          method: "GET",
        });
        if (!response.ok) {
          throw new Error("No se pudo obtener el actuador");
        }
        const data = await response.json();
        setDataActual(data);
      } catch (error) {
        console.log("Error al obtener este actuador", error);
      }
    };
    infoActuador();
  }, [slug]);
  ...
  const patchData = async (form) => {
    try {
      ...
      if (!response.ok) {
        throw new Error("ERROR al editar actuador");
      }
      const data = await response.json();
      console.log(data);
      setMessage(`Actuador editado correctamente`);
